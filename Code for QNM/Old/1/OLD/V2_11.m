...
capacity=500.*geo; % value can be changed
%demand = 3
parfor h = 6:15
    ...
    for i=1:8 % 8 origin and destination
        for j=1:8
            for k=1:t
                if i~=j
                    od(i,j,k)=3;%can be any value or stochastic
                end
            end
        end
    end
                ...
                if od(i,j,T)>0
                    ...
                    if a>1
                        if p(1,m+1)<p(2,m+1)
                            ...
                            if out(i,p(1,2),T+tc(i,p(1,2),T))>150
                                diff=out(i,p(1,2),T+tc(i,p(1,2),T))-150;
                                out(i,p(1,2),T+tc(i,p(1,2),T)+1)=out(i,p(1,2),T+tc(i,p(1,2),T)+1)+diff;
                                out(i,p(1,2),T+tc(i,p(1,2),T))=150; %outflow rate can be changed
                            ...
                            if out(i,p(2,2),T+tc(i,p(2,2),T))>150
                                diff=out(i,p(2,2),T+tc(i,p(2,2),T))-150;
                                out(i,p(2,2),T+tc(i,p(2,2),T)+1)=out(i,p(2,2),T+tc(i,p(2,2),T)+1)+diff;
                                out(i,p(2,2),T+tc(i,p(2,2),T))=150; %outflow rate can be changed
                ...
                if od(i,j,T)>0
                    ...
                    if a>1
                        if p(1,m+1)<p(2,m+1)
                            in(i,p(1,2),T)=in(i,p(1,2),T)+od(i,j,T);
                            out(i,p(1,2),T+tc(i,p(1,2),T))=out(i,p(1,2),T+tc(i,p(1,2),T))+od(i,j,T);
                            if out(i,p(1,2),T+tc(i,p(1,2),T))>100
                                diff=out(i,p(1,2),T+tc(i,p(1,2),T))-100;
                                out(i,p(1,2),T+tc(i,p(1,2),T)+1)=out(i,p(1,2),T+tc(i,p(1,2),T)+1)+diff;
                                out(i,p(1,2),T+tc(i,p(1,2),T))=100; %outflow rate can be changed
                                od(p(1,2),j,T+tc(i,p(1,2),T))=od(p(1,2),j,T+tc(i,p(1,2),T))+od(i,j,T)-diff;
                                od(p(1,2),j,T+tc(i,p(1,2),T)+1)=od(p(1,2),j,T+tc(i,p(1,2),T)+1)+diff;
                            else  
                            %if p(1,3)~=0
                            ...
                            %
                            out(i,p(2,2),T+tc(i,p(2,2),T))=out(i,p(2,2),T+tc(i,p(2,2),T))+od(i,j,T)/2;
                            if out(i,p(2,2),T+tc(i,p(2,2),T))>100
                                diff=out(i,p(2,2),T+tc(i,p(2,2),T))-100;
                                out(i,p(2,2),T+tc(i,p(2,2),T)+1)=out(i,p(2,2),T+tc(i,p(2,2),T)+1)+diff;
                                out(i,p(2,2),T+tc(i,p(2,2),T))=100; %outflow rate can be changed
                                od(p(2,2),j,T+tc(i,p(2,2),T))=od(p(2,2),j,T+tc(i,p(2,2),T))+od(i,j,T)/2-diff;
                                od(p(2,2),j,T+tc(i,p(2,2),T)+1)=od(p(2,2),j,T+tc(i,p(2,2),T)+1)+diff;
                            else  
                            %if p(1,3)~=0
