...
parfor ii = 1:256 % different cyclelength
    %define parameters
    ...
    phaseplan=[10.*ones(4,4) all(ii,:)'];  %can be changed, remain the same cycle length, and offset from 0 to 59
    signal=enSignal(phaseplan,phase,m,n,t+dt);

    %initial OD
    for i=1:2 % 8 origin and destination
        for j=1:8
            for mm=1:t
                if i~=j
                    od(i,j,mm)=3;%can be any value or stochastic
                end
            end
        end
   end
