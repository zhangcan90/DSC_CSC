...
%parfor h = 4:6 % different cyclelength
h=6;
    %define parameters
    ...
    for i=1:8 % 8 origin and destination
        for j=1:8
            for mm=1:t
                if i~=j
                    od(i,j,mm)=2;%can be any value or stochastic
                end
            end
        end
    end
            ...
            for j=1:n
                if od(i,j,T)>0
                    ...
                    if a>1
                        ...
                        %
%                         k=T;
%                         if outside(i,p(1,2),k+tc(i,p(1,2),k))>exit
%                             while outside(i,p(1,2),k+tc(i,p(1,2),k))>exit
%                                 diff=outside(i,p(1,2),k+tc(i,p(1,2),k))-exit;
%                                 outside(i,p(1,2),k+tc(i,p(1,2),k))=exit;
%                                 outside(i,p(1,2),k+1+tc(i,p(1,2),k+1))=outside(i,p(1,2),k+1+tc(i,p(1,2),k+1))+diff;
%                                 k=k+1;
...
%total travel time
    ...
    ttt=s1-s0;
    parsave(sprintf('outputwithpl%d.mat', h), in, out,od,ttt);
%end
%
%running time end
END=clock;
