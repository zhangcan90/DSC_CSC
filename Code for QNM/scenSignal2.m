function [green,red,greentime] = scenSignal2(phaseplan,phase,allred,m,n,t)
    ...
    for i=1:4
        for j=1:4
            if i==j
                ...
                red(6+i,6+j,T)=initial1(phase(i,j),mod(T-1,cyclelength)+1);
                red(10+i,10+j,T)=initial2(phase(i,j),mod(T-1,cyclelength)+1);
                red(14+i,14+j,T)=initial3(phase(i,j),mod(T-1,cyclelength)+1);
                red(18+i,18+j,T)=initial4(phase(i,j),mod(T-1,cyclelength)+1);
                green(6+i,6+j,T)=initialg1(phase(i,j),mod(T-1,cyclelength)+1);
                green(10+i,10+j,T)=initialg2(phase(i,j),mod(T-1,cyclelength)+1);
                green(14+i,14+j,T)=initialg3(phase(i,j),mod(T-1,cyclelength)+1);
                green(18+i,18+j,T)=initialg4(phase(i,j),mod(T-1,cyclelength)+1);
            end
        end
    end
end
...
for k=1:4
    ...
    greentime(4*k+3:4*k+6,4*k+3:4*k+6)=phasegreen;
end
